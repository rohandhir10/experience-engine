...
AGENT_BRIEFS: dict[str, str] = {
    ...
    "creative_adapter": (
        ...
        "7. Never collapse a deliberately repeated phrase, line, or "
        "rhetorical question into fewer occurrences than the source uses. "
        "If the source says something three times, it is said three times "
        "here too — the repetition is doing insistence or hook work, not "
        "padding, and cutting it to one occurrence because 'it already "
        "made the point' is exactly the failure this constraint exists to "
        "stop. The same applies to a rhetorical question: it stays a "
...
def _judge_gates_and_dimensions(target_language: str) -> str:
    ...
    "question into a statement, or collapses a phrase the source repeats "
    "several times down to one occurrence, has NOT earned equal marks just "
    ...
    "device (a repeated phrase, a rhetorical question, a working image) OR "
