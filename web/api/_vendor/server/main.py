"""VENDORED COPY for the Vercel deployment (web/api/engine.py imports
`app` from here). Canonical source: repo root server/main.py — keep
both in sync by hand until this deployment shape is replaced by
something that doesn't need a duplicate. See web/api/_vendor/README.md.
